    ...
    public class StudioEditor : Editor
    {
        ...
        private void DrawOutputFields()
        {
            ...
            if (studio.output.prefabMake)
            {
                ...
                if (studio.output.prefabMake && Model.IsMeshModel(SelectedModel))
                {
                    ...
                    {
                        EditorGUI.indentLevel++;
                        studio.output.locationSpritePrefab = EditorGUILayout.ObjectField(new GUIContent("Location Prefab",
            ...
            studio.output.normalMapMake = EditorGUILayout.Toggle("Make Normal Map", studio.output.normalMapMake);

            studio.output.materialMake = EditorGUILayout.Toggle("Make Material", studio.output.materialMake);
            if (studio.output.materialMake)
            {
                EditorGUI.indentLevel++;

                studio.output.spriteMaterial = EditorGUILayout.ObjectField(new GUIContent("Sprite Material",
                    "output sprite material to instantiate"), studio.output.spriteMaterial, typeof(Material), false) as Material;

                if (studio.output.spriteMaterial != null)
                {
                    EditorGUI.indentLevel++;
                    studio.output.materialBuilder = EditorGUILayout.ObjectField(new GUIContent("Material Builder",
                        "helps to construct output sprite material"), studio.output.materialBuilder, typeof(MaterialBuilder), false) as MaterialBuilder;
                    EditorGUI.indentLevel--;
                }
        ...
        private void UpdatePreviewTexture()
        {
            ...
            if (studio.output.normalMapMake && studio.preview.isNormalMap)
            {
                ...
                normalMapTexture = CapturingHelper.CaptureModelForNormalMap(SelectedModel, rotX, rotY, studio.shadow.obj);
            }
            ...
            if (studio.trimming.on)
            {
                ...
                previewTexture = TextureHelper.TrimTexture(previewTexture, texBound, studio.trimming.margin, EngineConstants.CLEAR_COLOR32);
                if (normalMapTexture != null)
                    normalMapTexture = TextureHelper.TrimTexture(normalMapTexture, texBound, studio.trimming.margin, EngineConstants.NORMALMAP_COLOR32, studio.output.normalMapMake);
            }
