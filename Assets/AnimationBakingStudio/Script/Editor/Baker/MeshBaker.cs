	...
	public class MeshBaker : Baker
	{
                    ...
                    if (studio.output.DoesMakeLocationPrefab())
                    {
                        ...
                        if (locations.Length > 0)
                        {
                            ...
                            for (int i = 0; i < locations.Length; ++i)
                            {
                                ...
                                if (loc.locationName != null && loc.locationName.Length > 0)
        ...
        public void OnBeginAnimation()
        {
            try
            {
                ...
                if (meshAnim.selectedFrames.Count > 0)
                {
                    frames = new Frame[meshAnim.selectedFrames.Count];
                    meshAnim.selectedFrames.CopyTo(frames);
                }
                else
                {
                ...
                if (meshModel.referenceController != null)
                    animationName = meshAnim.stateName;
                else
                    animationName = meshAnim.clip.name;
        ...
        public void OnBeginFrame()
        {
            try
            {
                ...
                meshModel.Animate(meshAnimations[AnimationIndex], frame);
        ...
        public void OnCaptureFrame()
        {
            try
            {
                ...
                if (studio.packing.on || studio.trimming.IsUnified())
                {
                    framePivots[frameIndex] = pivotClone;
                ...
                {
                    string materialFullName = BakeIndividually(ref modelTexture, pivotClone, viewName, frameIndex);
                    if (studio.output.normalMapMake)
                        BakeIndividually(ref normalMapTexture, pivotClone, viewName, frameIndex, true);
                    CreateMaterial(materialFullName, modelTexture, normalMapTexture);
                }
        ...
        public void OnEndView()
        {
            try
            {
                if (!studio.packing.on && studio.trimming.IsUnified())
                {
                    ...
                    BakeIndividually_Group(framePivots, viewName, frameModelTextures, frameNormalMapTextures);
                }
        ...
        public override void Finish()
        {
            ...
            meshModel.Animate(meshAnimations[0], Frame.BEGIN);
        }
