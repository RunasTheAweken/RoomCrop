...
namespace ABS
{
    public abstract class Baker
    {
        ...
        private Texture2D modelAtlasTexture;
        ...
        protected void TrimToUnifiedSize(IntegerVector[] pivots, Texture2D[] modelTextures, Texture2D[] normalMapTextures = null)
        {
            try
            {
                ...
                for (int i = 0; i < modelTextures.Length; ++i)
                {
                    ...
                    modelTextures[i] = TextureHelper.TrimTexture(modelTextures[i], unifiedTexBound, studio.trimming.margin, EngineConstants.CLEAR_COLOR32);
                    if (studio.output.normalMapMake)
                        normalMapTextures[i] = TextureHelper.TrimTexture(normalMapTextures[i], unifiedTexBound, studio.trimming.margin, EngineConstants.NORMALMAP_COLOR32, studio.output.normalMapMake);
                }
            }
            catch (Exception e)
            {
        ...
        protected void BakeIndividually_Group(IntegerVector[] pivots, string subName, Texture2D[] modelTextures, Texture2D[] normalMapTextures = null)
        {
            try
            {
                for (int i = 0; i < modelTextures.Length; i++)
                {
                    string materialFullName = BakeIndividually(ref modelTextures[i], pivots[i], subName, i);
                    if (studio.output.normalMapMake)
                        BakeIndividually(ref normalMapTextures[i], pivots[i], subName, i, true);
                    CreateMaterial(materialFullName, modelTextures[i], normalMapTextures[i]);
                }
            }
            catch (Exception e)
            {
        ...
        protected string BakeIndividually(ref Texture2D tex, IntegerVector pivot, string subName, int frame, bool isNormalMap = false)
        {
            try
            {
        ...
        protected string BakeIndividuallyReal(ref Texture2D tex, IntegerVector pivot, string subName, string detailName, bool isNormalMap = false)
        {
            try
            {
        ...
        protected Sprite[] BakeWithPacking(IntegerVector[] pivots, string subName, string[] spriteNames, Texture2D[] modelTextures, Texture2D[] normalMapTextures = null)
        {
            ...
            try
            {
                ...
                {
                    MakeAtlasInOrder(modelTextures, ref modelAtlasTexture, ref atlasLength, ref atlasRects, EngineConstants.CLEAR_COLOR32);
                    if (studio.output.normalMapMake)
                        MakeAtlasInOrder(normalMapTextures, ref normalMapAtlasTexture, ref atlasLength, ref atlasRects, EngineConstants.NORMALMAP_COLOR32);
                }
                ...
                atlasMaterial = CreateMaterial(fileFullName, modelAtlasTexture, normalMapAtlasTexture);
        ...
        protected Material CreateMaterial(string fileName, Texture2D modelTex, Texture2D normalMapTex)
        {
            ...
            if (studio.output.materialMake)
            {
                if (studio.output.spriteMaterial != null)
                {
                    ...
                    if (studio.output.materialBuilder != null)
                        studio.output.materialBuilder.BindTextures(material, modelTex, normalMapTex);
                }
                else
                {
                    Shader shader;
                    ...
                    if (shader != null)
                    {
                        ...
                        material.SetTexture("_MainTex", modelTex);
                        if (normalMapTex != null)
                            material.SetTexture("_BumpMap", normalMapTex);
                    }
                }
        ...
        protected AnimationClip MakeAnimationClipsForView(bool isLooping, Sprite[] sprites, string viewName)
        {
            ...
            {
                EditorCurveBinding materialCurveBinding = PrefabBuilder.GetMaterialCurveBinding();

                ObjectReferenceKeyframe[] materialKeyFrames = new ObjectReferenceKeyframe[1];
                materialKeyFrames[0] = new ObjectReferenceKeyframe
                {
                    time = 0,
                    value = atlasMaterial
                };

                AnimationUtility.SetObjectReferenceCurve(animClip, materialCurveBinding, materialKeyFrames);
            }
